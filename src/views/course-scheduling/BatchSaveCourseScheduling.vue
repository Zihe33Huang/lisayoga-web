<template>
  <div>
    <el-dialog :title="$t('batch_add')" width="850px"
      ...
      <el-form id="batch-save-course-scheduling-form" ref="form" :model="form" :rules="rules" label-width="100px" inline class="tams-form-container">
        <el-form-item :label="$t('classroom')" prop="classroomId">
        ...
        <br/>
        <el-form-item :label="$t('course')" prop="courseId">
        ...
        <el-form-item :label="$t('teacher')" prop="teacherId">
        ...
        <el-form-item :label="$t('start_end_dates')" prop="dates">
          <el-date-picker :clearable="false"
                          v-model="form.dates"
                          :picker-options="datesPickerOptions"
                          type="daterange"
                          :range-separator=" $t('to') " style="width: 610px;">
          </el-date-picker>
        </el-form-item>
        <el-form-item :label="$t('weekdays')" prop="weekList">
          <el-checkbox-group v-model="weekList" :min="1" size="small">
            <el-checkbox border v-for="item in weekOptions" :key="item.label" :label="item.label" :checked="item.checked">{{$t('week_options.' + item.name)}}</el-checkbox>
          </el-checkbox-group>
        </el-form-item>
        <br/>
        <el-form-item :label="$t('start_time')" prop="attendTime">
        ...
        <el-form-item :label="$t('end_time')" prop="finishTime">
      ...
      <div slot="footer" class="dialog-footer">
        ...
        <el-button type="primary" :loading="submitBtnLoading" @click="submit">{{$t('confirm')}}</el-button>
      </div>
    </el-dialog>
...
export default {
  ...
  data () {
    return {
      ...
      weekOptions: [
        { label: 1, name: 'monday' },
        { label: 2, name: 'tuesday' },
        { label: 3, name: 'wednesday' },
        { label: 4, name: 'thursday' },
        { label: 5, name: 'monday' },
        { label: 6, name: 'saturday' },
        { label: 7, name: 'sunday' }
      ],
      datesPickerOptions: {
        firstDayOfWeek: 1
