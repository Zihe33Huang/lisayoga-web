...
export default {
  ...
  watch: {
    '$i18n.locale' (newLocale, oldLocale) {
      ...
      this.$set(this.calendarOptions, 'locale', newLocale)
    }
  },
  data () {
    return {
        ...
        if (res) {
          ...
          res.forEach(item => {
            this.calendarOptions.events.push({
              id: item.id,
              title: item.courseName + ' ' + item.classroomName + ' ' + item.teacherName + ' ' + item.signInNum + '/' + item.appointNum + '/' + item.maximum,
      ...
      if (info.view.type === 'dayGridMonth') {
        ...
        return {
          html: `<div class="fc-scrollgrid-sync-inner">
                    ...
                    <span id="tams-course-count-${moment(info.date).format('YYYY-MM-DD')}" style="cursor: default;">${this.getCourseCount(info.date)}</span>
                </div>`
        }
  ...
  mounted () {
    this.init()
    this.$set(this.calendarOptions, 'locale', this.$i18n.locale === 'cn' ? 'zh' : 'en')
  }
}
</script>
